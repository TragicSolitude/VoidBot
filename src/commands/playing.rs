...
impl Command for Playing {
    fn execute(&self, _: &mut Context, msg: &Message, _: Args) -> Result<(), Error> {
        ...
        let name: String = msg.content.split(" ").skip(1).collect::<Vec<&str>>().join(" ");
        match CURRENT_SERVER.lock().get(&msg.author.id) {
            Some(guild) => {
                if let Some(guild_id) = guild {
                    let new_channel = guild_id.create_channel(&name, ChannelType::Voice, None)?;
                    MANAGED_CHANNELS.lock()?.push(ManagedChannel::new(new_channel.id));
